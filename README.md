# 🛠️ 현수의 작업실 (Studio)[cite: 3]

**현수의 작업실**은 SKALA 과정에서 진행하는 웹 프론트엔드 프로젝트와 실험들을 기록하는 개인 포트폴리오 공간입니다[cite: 3]. 순수 HTML, CSS, JavaScript(Vanilla JS)를 활용하여 웹 페이지의 구조를 설계하고, 동적인 인터랙션과 레이아웃을 구현하는 방법을 학습하고 적용했습니다[cite: 1, 3, 10].

---

## 📌 주요 페이지 및 핵심 문법 (Syntax) 분석

### 1. 🏠 메인 허브 (`index.html`) - 시맨틱 마크업과 Flexbox 레이아웃[cite: 3]
웹 페이지의 전체적인 뼈대를 잡고, 콘텐츠를 논리적인 영역으로 나누어 배치했습니다[cite: 3].

* **HTML 시맨틱 태그 (Semantic Tags)**[cite: 3]
  문서의 구조를 브라우저와 개발자가 명확하게 이해할 수 있도록 의미 있는 태그를 사용했습니다.
  ```html
...
* **CSS Flexbox를 이용한 레이아웃 분할**[cite: 1]
...
### 2. 👤 나의 프로필 (`myProfile.html`) - 인터랙티브 UI와 목록 태그[cite: 6]
JS 없이 HTML 태그만으로 열고 닫히는 UI를 만들고, 다양한 리스트 태그를 적용했습니다[cite: 6].

* **아코디언 토글 UI (`<details>`, `<summary>`)**[cite: 6]
...
### 3. 📅 나의 수업 (`myClass.html`) - 2차원 테이블(표) 설계[cite: 4]
일주일 간의 시간표를 만들기 위해 테이블 태그의 병합 속성을 적극 활용했습니다[cite: 4].

* **테이블 셀 병합 (`rowspan`, `colspan`)**[cite: 4]
  ```html
...
### 4. 🏖️ 나의 휴일 (`myHoliday.html`) - 상태 선택자 기반 애니메이션[cite: 5]
체크박스 클릭 상태에 따라 인접한 텍스트의 디자인이 바뀌는 '체크리스트'를 구현했습니다[cite: 1, 5].

* **형제 선택자와 가상 클래스 (`:checked + label`)**[cite: 1]
...
### 5. ✈️ 나의 여행 일기 (`myTrip.html`) - CSS Grid와 미디어[cite: 7]
다양한 미디어를 삽입하고, 화면 크기에 맞춰 카드가 자동 배열되는 그리드 갤러리를 만들었습니다[cite: 1, 7].

* **반응형 CSS Grid Layout**[cite: 1]
...
### 6. 📝 회원가입 (`signUp.html`) - 폼(Form) 유효성 검사[cite: 8]
클라이언트(브라우저) 단에서 입력 데이터의 형식을 검사하는 HTML5 문법을 적용했습니다[cite: 8].

* **정규식(RegEx) 및 속성을 활용한 제약 조건**[cite: 8]
  ```html
...
### 🎒 내 가방 보기 (`bag.js`)[cite: 10]
복합 데이터(배열과 객체)를 다루고 반복문을 통해 데이터를 처리하는 로직입니다[cite: 10].

* **배열(Array) 내부의 객체(Object) 선언**[cite: 10]
  ```javascript
  var myBag = [
      { name: "노트북 💻", count: 1 },
      { name: "필통 ✏️", count: 1 }
